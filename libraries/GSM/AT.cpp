...
{
  Serial2.print(long_value);
}

void AT::Println(long long_value)
{
  Serial2.println(long_value);
}
...
{
  return (Serial2.readBytesUntil(terminator, buffer, length));
}
...
bool AT::FindUntil(char *target, char *terminator, unsigned long timeout)
{
  ...
  return (Serial2.findUntil(target, terminator));
}
